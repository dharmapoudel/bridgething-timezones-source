...
1. Push this repo to `https://github.com/dharmapoudel/bridgething-timezones-source`.
2. In **Settings > Pages**, set the source to **Deploy from a branch**, branch `gh-pages`, folder `/ (root)`.

The catalog is published to `https://dharmapoudel.github.io/bridgething-timezones-source/catalog.v1.json`, which can be submitted to <bridgething.com/apps>
...
With more than one app in `apps/` your commands must specify which one: `bun run dev bridgething-timezones`.

Screenshot for the store listing:

```sh
bun run shot bridgething-timezones            # grabs what is on the screen
bun run shot bridgething-timezones --replace  # overwrite
```
...
```sh
bun run bump bridgething-timezones patch -m "Fix the wind direction arrow"
git commit -am "bridgething-timezones: fix the wind direction arrow" && git push
```
