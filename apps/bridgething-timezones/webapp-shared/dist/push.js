...
import { bundleDirName, resolveGatewayTarget, setSlot, switchTo, uuidToString } from './gateway.js';
...
function pipeThrough(producer, consumer, label) {
    return new Promise((res, rej) => {
        ...
        source.on('exit', code => {
            if (code !== 0)
                rej(new Error(`${producer.cmd} exited ${code}`));
        });
...
async function pushBundle(localDir, host, dirName) {
    ...
    await pipeThrough({ cmd: 'tar', args: ['-czf', '-', '-C', localDir, '.'] }, { cmd: 'ssh', args: [...SSH_OPTS, `root@${host}`, receive] }, 'ssh');
}
...
function parseArgs(argv) {
    ...
    for (const arg of argv) {
        if (arg === '--skip-build')
            ...
            args.switchAfter = false;
        else if (arg === '--switch')
            args.switchAfter = true;
        else if (arg === '--no-slot')
            args.claimSlots = false;
        else if (arg === '--release')
            args.release = true;
...
export async function bridgethingPush({ scriptUrl }) {
    ...
    if (args.release) {
        ...
        if (declaredSlots(manifest).length === 0)
            console.log('this bundle declares no slots; nothing to release');
        return;
    }
    if (!args.skipBuild)
