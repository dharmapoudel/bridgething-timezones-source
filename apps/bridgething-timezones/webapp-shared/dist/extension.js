...
import { GatewayLink, getActive, getNickname, listConfig, parseUuid, uuidToString, } from './gateway.js';
...
export function readManifest(publicDir) {
    const manifest = JSON.parse(readFileSync(join(publicDir, 'manifest.json'), 'utf8'));
    if (!manifest.id)
        throw new Error('public/manifest.json has no id');
    return manifest;
}
export function extensionBuildOptions(source, outfile) {
    return {
        entryPoints: [source],
        outfile,
        bundle: true,
...
export async function buildExtension(root, outDir, extension) {
...
export async function resolveDeno(root) {
    ...
    if (packaged) {
        ...
        if (!existsSync(exe))
            await runToExit(process.execPath, [join(packaged, 'bin.cjs'), '--version'], root);
        if (existsSync(exe))
            return exe;
    }
    for (const dir of (process.env.PATH ?? '').split(delimiter)) {
        if (dir && existsSync(join(dir, exeName)))
            return join(dir, exeName);
    }
    throw new Error(`deno is not installed; \`bun add -d deno@${DENO_PACKAGE_VERSION}\` puts the runtime the desktop app uses into node_modules`);
}
...
export class ExtensionDevHost {
    ...
    spawnChild() {
        ...
        createInterface({ input: process_.stderr }).on('line', line => {
            if (line.trim())
                this.log.warn(`[${this.appName()} stderr] ${line}`);
        });
        ...
        if (this.device)
            this.write(child, this.connected(this.device));
    }
    ...
    exited(generation, code, signal) {
        ...
        if (!child || child.generation !== generation)
            return;
        this.child = null;
        if (this.waiting?.generation === generation)
            this.waiting = null;
        if (child.ready)
            this.publishRunning([]);
        if (this.closing || child.intent === 'stop')
            return;
        ...
        this.crashes += 1;
        this.log.error(`extension exited (${signal ?? `code ${code}`}); restarting in ${Math.round(delay / 1000)}s, or on the next save`);
    ...
    reply(generation, message) {
        if (this.child?.generation === generation)
            this.write(this.child, message);
    }
    ...
    sendToDevice(device, message) {
        ...
        if (!target || !this.link?.isOpen)
            return;
        if (device !== undefined ? device !== target.id : !target.active)
            return;
        this.link.event({
            type: 'forward',
    ...
    async runLink() {
        ...
        while (!this.closing) {
            ...
            try {
                ...
                if (failures === 0) {
                    this.log.warn(`no car thing at ${this.opts.target.name}:${this.opts.target.port} (${err instanceof Error ? err.message : String(err)}); the extension runs and links up when one appears`);
                }
                failures += 1;
            ...
            if (this.device) {
                ...
                this.device = null;
                if (this.child)
                    this.write(this.child, { t: 'device.disconnected', device: gone.id });
            }
            if (this.closing)
                break;
    ...
    async linked(link) {
        ...
        if (!link.isOpen || this.closing)
            return;
        const [active, config, nickname] = results;
        if (active.status === 'rejected' || config.status === 'rejected') {
            const failure = active.status === 'rejected' ? active.reason : config.status === 'rejected' ? config.reason : null;
            this.log.error(`could not read the car thing's state (${String(failure)}); dropping the link to retry`);
            link.close();
            return;
        }
        if (!active.value.ok)
            this.log.warn(`active webapp unknown: ${active.value.reason}`);
        if (!config.value.ok)
            this.log.warn(`config unreadable: ${config.value.reason}`);
        ...
        this.device = device;
        this.log.info(`car thing linked (${device.name}); ${device.active ? 'this app is active, forwards flow both ways' : 'this app is not active, so forwards will not route until it is'}`);
        if (this.child)
            this.write(this.child, this.connected(device));
        if (this.child?.ready)
            this.publishRunning([this.id]);
    }
    fromDevice(data) {
        ...
        if (outer?.type === 'webapp' && outer.data?.event === 'activeChanged') {
            ...
            this.log.info(active ? 'this app is now active on the car thing' : 'this app is no longer active on the car thing');
            if (this.child)
                this.write(this.child, { t: 'device.active', device: device.id, active });
            return;
        }
        if (outer?.type === 'webapp' && outer.data?.event === 'configChanged') {
            const changed = outer.data.data;
            if (uuidToString(changed.id) !== this.id)
                return;
            if (changed.value === null)
                delete device.config[changed.key];
            else
                device.config[changed.key] = changed.value;
            if (this.child) {
        ...
        if (outer?.type === 'forward' && outer.data?.event === 'routed') {
            ...
            if (uuidToString(routed.webapp) !== this.id || !this.child)
                return;
