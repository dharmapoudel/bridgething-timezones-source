...
import { buildExtension, ExtensionDevHost, readManifest } from './extension.js';
import { deviceHostName, listWebapps, navigateKiosk, resolveGatewayTarget, resolveHost, switchTo, uuidToString, } from './gateway.js';
...
export async function daemonProxyTarget() {
    ...
    return {
        ...
        configureServer(server) {
            ...
            try {
                ...
                log.warn(`bridgething: ${err instanceof Error ? err.message : String(err)}; device mode and extensions are off`);
                return;
            }
            const extension = manifest.extension;
            if (extension)
                server.middlewares.use(AUTHORIZE_PATH, authorizePage(() => host));
            http.once('listening', () => {
                void (async () => {
                    const target = await resolveGatewayTarget();
                    try {
                        if (mode === DEVICE_MODE)
                            device = await attachDevice(server, root, manifest, target, log);
                        else if (extension)
                            await makeActive(root, manifest, target, log);
                    }
                    catch (err) {
            ...
            server.close = async () => {
                ...
                if (ending) {
                    await ending.release().catch((err) => {
                        log.warn(`device mode: could not hand the screen back: ${err instanceof Error ? err.message : String(err)}`);
                    });
                }
                await close();
            };
...
async function attachDevice(server, root, manifest, target, log) {
    if (!isIPv4(target.host))
        throw new Error(`could not resolve ${target.name}; is the car thing plugged in?`);
    const local = hostAddressToward(target.host);
    if (!local)
        throw new Error(`no interface on this machine shares a subnet with ${target.name} (${target.host})`);
    const address = server.httpServer?.address();
    if (!address)
        throw new Error('the dev server is not listening');
    ...
    log.info(`car thing is showing ${url}; edits hot-reload on the device, ctrl-c hands the screen back to the installed build`);
    return {
        async release() {
            await activate(target, manifest.id);
        },
...
async function makeActive(root, manifest, target, log) {
    ...
    try {
        ...
        log.warn(`could not make ${name} active on the car thing (${err instanceof Error ? err.message : String(err)}); forwards only route while it is`);
    }
}
async function installOnce(root, manifest, target, log) {
    if (await installed(target, manifest.id))
        return;
...
function hostAddressToward(ip) {
    ...
    for (const entries of Object.values(networkInterfaces())) {
        for (const entry of entries ?? []) {
            if (entry.family !== 'IPv4' || entry.internal || !entry.cidr)
                continue;
            const prefix = Number(entry.cidr.split('/')[1]);
            const own = ipv4Bits(entry.address);
            if (own === null || !Number.isFinite(prefix))
                continue;
            const mask = prefix === 0 ? 0 : (0xffffffff << (32 - prefix)) >>> 0;
            if ((own & mask) >>> 0 === (wanted & mask) >>> 0)
...
function authorizePage(host) {
    return (req, res, next) => {
        ...
        req.on('end', () => {
            ...
            let note;
            if (!current)
                note = 'no extension is running';
            else if (form.has('cancel'))
                note = current.cancelAuthorize() ? 'cancelled' : 'nothing was waiting';
            else {
                const callback = normalizeCallback(form.get('url') ?? '');
                if (!callback)
                    note = 'that is not a url';
                else
                    note = current.settleAuthorize(callback) ? 'delivered to the extension' : 'nothing was waiting';
            }
            res.setHeader('content-type', 'text/html; charset=utf-8');
